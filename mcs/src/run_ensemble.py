...
if __name__ == '__main__':
    ...
    print(time.asctime(), f'Inferring experimental results from saved pt files')
    ...
    fold = '../Exp_us_' + forecast_date
    seed_range = [0, 1, 2]
    lead = []
    for seed in seed_range:
        dest_fold = fold + '/confirmed_7_hierst_seed' + str(seed) + '/log.txt'
        with open(dest_fold, 'rb') as f:
            lines = f.readlines()
            last_line = float(lines[-1].decode()[-8:])
            lead.append(last_line)
    final_mape = sum(lead) / 3
    print('Final Result is ' + str(final_mape))
